local function config()
	...
	local function button(sc, txt, keybind, keybind_opts)
		...
		if keybind then
			...
			opts.keymap = { "n", sc, keybind, { noremap = false, silent = true, nowait = true } }
		end
	...
	local function drawSessions()
		...
		for i, fn in ipairs(sessions) do
			...
			local btn = button(
				sc,
				file,
				'<cmd>lua require("session_manager.utils").load_session("' .. fn.filename .. '")<CR>'
			)
		...
		mru = {
			...
			val = {
				...
				{ type = "padding", val = 1 },
				{
					type = "group",
					val = function()
						return { mru(vim.tbl_count(sessions)) }
					end,
				},
			},
		},
...
local function init(use)
	return use({
		"goolord/alpha-nvim",
		config = function()
			require("kp.config.alpha").config()
		end,
